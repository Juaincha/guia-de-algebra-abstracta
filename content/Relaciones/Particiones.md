...
>
>Por último, como cada elemento de $A$ pertenece a un único miembro de $P$, no existe ningún $a \in A$ tal que existan $M,N \in P$ distintos tales que $a \in M$ y $a \in N$. Por las leyes de negación, esto es lo mismo que decir que para todo $a \in A$ y para todo $M,N \in P$ distintos no se cumple $a \in M$ y $a \in N$. Esto es, para todo $a \in A$ y para todo $M,N \in P$ distintos tenemos $a \notin M \cap N$. Como $M$ y $N$ son subconjuntos de $A$, decir que para todo $a \in A$ $a \notin M \cap N$ es equivalente a $M \cap N = \emptyset$. Luego para todo $M,N \in P$ $M \neq N \implies M \cap N = \emptyset$. Luego si $P$ es partición de $A$, se cumplen las tres propiedades.
>
